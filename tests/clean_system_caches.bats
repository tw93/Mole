...
setup() {
    ...
    rm -f "$HOME/.cache/roomy/permissions_granted"
}
...
@test "check_tcc_permissions skips in non-interactive mode" {
    ...
    [ "$status" -eq 0 ]
    [[ ! -f "$HOME/.cache/roomy/permissions_granted" ]]
}

@test "check_tcc_permissions skips when permissions already granted" {
    mkdir -p "$HOME/.cache/roomy"
    touch "$HOME/.cache/roomy/permissions_granted"
...
set -euo pipefail
printf '%s\n' "$HOME/CustomProjects" > "$HOME/.config/roomy/purge_paths"
source "$PROJECT_ROOT/lib/core/common.sh"
source "$PROJECT_ROOT/lib/clean/caches.sh"
run_with_timeout() { shift; "$@"; }
safe_clean() { echo "$2|$1"; }
...
EOF
    ...
    rm -rf "$HOME/CustomProjects" "$HOME/.config/roomy" "$fake_bin" "$find_log"
}
...
@test "discover_project_cache_roots dedupes aliased roots by filesystem identity" {
    ...
    printf '%s\n' "$HOME/Code" > "$HOME/.config/roomy/purge_paths"
...
@test "clean_project_caches skips stalled root scans" {
    ...
    printf '%s\n' "$HOME/SlowProjects" > "$HOME/.config/roomy/purge_paths"
...
export ROOMY_PROJECT_CACHE_DISCOVERY_TIMEOUT=0.5
export ROOMY_PROJECT_CACHE_SCAN_TIMEOUT=0.5
SECONDS=0
clean_project_caches
...
EOF
    ...
    rm -rf "$HOME/.config/roomy" "$HOME/SlowProjects" "$fake_bin"
}
