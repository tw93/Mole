...
@test "clean_user_essentials keeps Roomy runtime logs while cleaning other user logs" {
    ...
    touch "$HOME/Library/Logs/roomy/operations.log"
...
[[ -f "$HOME/Library/Logs/roomy/operations.log" ]]
...
@test "clean_apple_silicon_caches skips system Rosetta cache without system cleanup" {
    ...
    run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" ROOMY_ROSETTA_UPDATE_BUNDLE_PATH="$rosetta_path" bash --noprofile --norc <<'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
source "$PROJECT_ROOT/lib/clean/user.sh"
IS_M_SERIES=true
...
export ROOMY_EXTERNAL_VOLUMES_ROOT="$link_root"
