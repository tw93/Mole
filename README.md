...
A size ending in `+` contains measured bytes from a partial scan; `unknown` means the size could not be measured. Unreadable directories stay visible, and incomplete results do not replace complete cached measurements. A later refresh can recover the missing data.
