...
[[ -z "${ROOMY_COMMON_LOADED:-}" ]] && source "$SCRIPT_DIR/lib/core/common.sh"
...
stop_launch_services() {
    ...
    if [[ "$has_system_files" == "true" && "${ROOMY_TEST_MODE:-0}" != "1" && "${ROOMY_TEST_NO_AUTH:-0}" != "1" ]]; then
        if [[ -d /Library/LaunchAgents ]]; then
            while IFS= read -r -d '' plist; do
...
# Performance: when ROOMY_DELETE_MODE=trash and the batch is sudo-free and
...
# terminal" reported during `roomy uninstall` on apps with many leftovers.
remove_file_list() {
    local file_list="$1"
    ...
    while IFS= read -r file; do
        ...
        # Symlinks and sudo-required paths stay on the per-file roomy_delete
    ...
    if [[ ${#trash_batch[@]} -gt 0 ]]; then
        if _roomy_move_to_trash_batch "${trash_batch[@]}"; then
            local _bp _bsize
            for _bp in "${trash_batch[@]}"; do
                _bsize="unknown"
                _roomy_delete_log "trash" "$_bsize" "ok" "$_bp"
                log_operation "${ROOMY_CURRENT_COMMAND:-uninstall}" "TRASHED" "$_bp" "batch"
            done
            count=$((count + ${#trash_batch[@]}))
        else
            # Batch failed wholesale: route each path through roomy_delete so
...
batch_uninstall_applications() {
    ...
    trap 'stop_inline_spinner 2>/dev/null; _cleanup_sudo_keepalive; unset ROOMY_UNINSTALL_MODE; echo ""; _restore_uninstall_traps; return 130' INT TERM
    ...
    # does not work reliably under Roomy's timed non-interactive execution path.
    if [[ "${ROOMY_DRY_RUN:-0}" != "1" ]] &&
    ...
    for detail in "${app_details[@]}"; do
        ...
        if [[ -z "$reason" ]]; then
            if [[ "$is_brew_cask" == "true" && -n "$cask_name" ]]; then
                ...
                if brew_uninstall_cask "$cask_name" "$app_path"; then
                    ...
                    # where brew still reports the app as installed after Roomy
                    ...
                    if [[ $cask_state -eq 1 ]]; then
                        if ! roomy_delete "$app_path" "$needs_sudo"; then
                ...
                if [[ -L "$app_path" ]]; then
                    ...
                    if [[ -n "$link_target" ]]; then
                        ...
                        case "$resolved_target" in
                            /System/* | /usr/bin/* | /usr/lib/* | /bin/* | /sbin/* | /private/etc/*)
                                ...
                                ;;
                            *)
                                if ! roomy_delete "$app_path" "true"; then
                    ...
                    if is_uninstall_dry_run; then
                        if ! roomy_delete "$app_path" "false"; then
                            reason="dry-run path validation failed"
                        fi
                    else
                        local ret=0
                        roomy_delete "$app_path" "true" || ret=$?
                        if [[ $ret -ne 0 ]]; then
                            local diagnosis
        ...
        if [[ -z "$reason" ]]; then
            ...
            if [[ -n "$bundle_id" && "$bundle_id" != "unknown" ]]; then
                ...
                # User-owned plists, so route through user-mode roomy_delete to
                ...
                if [[ -d "$HOME/Library/Preferences/ByHost" ]]; then
                    if [[ "$bundle_id" =~ ^[A-Za-z0-9._-]+$ ]]; then
                        while IFS= read -r -d '' plist_file; do
                            roomy_delete "$plist_file" "false" || true
    ...
    if [[ ${#local_network_warning_apps[@]} -gt 0 ]]; then
        ...
        summary_details+=("${GRAY}${ICON_SUBLIST}${NC} Roomy does not reset ${GRAY}/Volumes/Data/Library/Preferences/com.apple.networkextension*.plist${NC}")
    ...
    if [[ $brew_apps_removed -gt 0 && "${ROOMY_DRY_RUN:-0}" != "1" ]]; then
        (
