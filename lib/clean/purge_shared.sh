...
if [[ -n "${ROOMY_PURGE_SHARED_LOADED:-}" ]]; then
    return 0
fi
readonly ROOMY_PURGE_SHARED_LOADED=1

# Canonical purge targets (heavy project build artifacts).
readonly ROOMY_PURGE_TARGETS=(
    "node_modules"
    "target"        # Rust, Maven
...
readonly ROOMY_PURGE_DEFAULT_SEARCH_PATHS=(
    "$HOME/www"
    "$HOME/dev"
    "$HOME/Projects"
    "$HOME/GitHub"
...
readonly ROOMY_PURGE_MONOREPO_INDICATORS=(
    "lerna.json"
    "pnpm-workspace.yaml"
    "nx.json"
    "rush.json"
...
readonly ROOMY_PURGE_PROJECT_INDICATORS=(
    "package.json"
    "Cargo.toml"
    "go.mod"
    "pyproject.toml"
...
readonly ROOMY_CACHEDIR_TAG_NAME="CACHEDIR.TAG"
readonly ROOMY_CACHEDIR_TAG_SIGNATURE="Signature: 8a477f597d28d172789f06886806bc55"

# High-noise targets intentionally excluded from quick hint scans in roomy clean.
readonly ROOMY_PURGE_QUICK_HINT_EXCLUDED_TARGETS=(
    "bin"
    "vendor"
)

roomy_purge_is_project_root() {
    local dir="$1"
    local indicator

    for indicator in "${ROOMY_PURGE_MONOREPO_INDICATORS[@]}"; do
...
roomy_dir_has_cachedir_tag() {
    local dir="$1"
    local tag="$dir/$ROOMY_CACHEDIR_TAG_NAME"
    [[ -f "$tag" && ! -L "$tag" ]] || return 1

    local signature
    signature=$(LC_ALL=C dd bs=${#ROOMY_CACHEDIR_TAG_SIGNATURE} count=1 < "$tag" 2> /dev/null || true)
    [[ "$signature" == "$ROOMY_CACHEDIR_TAG_SIGNATURE" ]]
}

roomy_purge_quick_hint_target_names() {
    local target
    local excluded
    local is_excluded

    for target in "${ROOMY_PURGE_TARGETS[@]}"; do
        is_excluded=false
        for excluded in "${ROOMY_PURGE_QUICK_HINT_EXCLUDED_TARGETS[@]}"; do
