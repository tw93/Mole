...
require_match "LAUNCH_READINESS.md" '^## Goal 10: Open Source Compliance And Namespace$' "open-source compliance goal must be documented"
...
require_match "scripts/run-clean-machine-cli-drill.sh" 'raw[.]githubusercontent[.]com/jake-seo-cl/roomy/\$\{tag\}/install[.]sh' "automated clean-machine drill must download the tag-specific install script"
...
require_match "scripts/run-clean-machine-cli-drill.sh" 'raw[.]githubusercontent[.]com/jake-seo-cl/roomy/\$\{previous_tag\}/install[.]sh' "automated clean-machine drill must download the previous stable installer"
...
require_match ".github/workflows/release.yml" '\$\{GITHUB_REPOSITORY,,\}.*jake-seo-cl/roomy' "release workflow must restrict Homebrew publishing to canonical repository"
...
require_match ".github/workflows/release.yml" '--homebrew-package jake-seo-cl/tap/roomy' "release workflow must validate the release tap formula before stable publication"
require_match ".github/workflows/release.yml" 'https://github[.]com/jake-seo-cl/roomy/releases/download/\$\{TAG\}/\$\{evidence_asset\}' "release workflow must write canonical clean-machine evidence asset URLs"
...
require_match "scripts/update_homebrew_tap_formula.sh" 'https://github.com/jake-seo-cl/roomy/archive/refs/tags' "Homebrew tap updater must emit canonical lowercase source archive URLs"
require_match "scripts/update_homebrew_tap_formula.sh" 'https://github.com/jake-seo-cl/roomy/releases/download' "Homebrew tap updater must emit canonical lowercase binary release URLs"
...
require_match "docs/launch/go-no-go-audit.md" 'Open-source compliance is preserved' "go/no-go audit must require open-source compliance evidence"
require_match "docs/release/release-notes-template.md" '^# Roomy <TAG> Release Notes$' "release notes template must exist"
require_match "docs/release/release-notes-template.md" '^## License And Source$' "release notes template must include license/source section"
...
require_match "scripts/check-release-notes.sh" 'License And Source' "release notes verifier must require license/source section"
...
require_match ".github/workflows/release.yml" 'License: GPL-3[.]0' "release manifest must state GPL license"
require_match ".github/workflows/release.yml" 'Corresponding source: https://github[.]com/\$\{GITHUB_REPOSITORY\}/tree/\$\{TAG\}' "release manifest must link corresponding source"
...
require_match "docs/marketing/pricing-strategy.md" '^# Pricing Strategy$' "pricing strategy must exist"
require_match "docs/marketing/pricing-strategy.md" 'Keep the Roomy CLI free and open source under GPL-3[.]0' "pricing strategy must preserve GPL open-source positioning"
require_match "docs/legal/open-source-compliance.md" '^# Open Source Compliance$' "open-source compliance checklist must exist"
require_match "docs/legal/open-source-compliance.md" 'GPL-3[.]0-only derivative' "open-source compliance checklist must state conservative GPL posture"
require_match "docs/launch/distribution-automation.md" '^# Distribution Automation$' "distribution automation plan must exist"
require_match "docs/launch/distribution-automation.md" 'Canonical source repository: .jake-seo-cl/roomy.' "distribution automation must name the Roomy namespace"
require_match "scripts/check-license-compliance.sh" 'LICENSE must contain GPL-3[.]0 text' "license compliance checker must validate GPL license text"
require_match "scripts/release-preflight.sh" 'check-license-compliance[.]sh' "release preflight must run license compliance checks"
...
require_match "site/scripts/check-site.mjs" 'same GPL CLI' "landing smoke check must enforce GPL-compatible pricing copy"
